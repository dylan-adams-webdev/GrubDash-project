	...
	describe('update method', () => {
		...
		test('returns 400 if description is missing', async () => {
			const original = {
				...
				name: 'Century Eggs',
				description:
					'Whole eggs preserved in clay and ash for a few months',
				price: 17,
			};
		...
		test('returns 400 if price is less than zero', async () => {
			...
			const response = await request(app)
				...
				.set('Accept', 'application/json')
				.send({
					data: { ...original, price: Number.MIN_SAFE_INTEGER },
				});
			...
			const expected = [
				{
					...
					name: 'Witchetty Grub',
					description:
						'The larva of a moth that feeds on the Witchetty bush',
					price: 20,
				},
				{
					id: '21',
